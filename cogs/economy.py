...
class Economy:
    ...
    async def register(self, ctx):
        """Registers an account at the Wolfstorm bank"""
    ...
    async def _set(self, ctx, user: discord.Member, credits: SetParser):
        ...
        Examples:
            bank set @Wolfstorm 26 - Sets 26 credits
            bank set @Wolfstorm +2 - Adds 2 credits
            bank set @Wolfstorm -6 - Removes 6 credits"""
        author = ctx.message.author
        try:
...
def setup(bot):
    ...
    check_folders()
    check_files()
    logger = logging.getLogger("chronoxia.economy")
    if logger.level == 0:

...
default_path = "data/chronoxia/settings.json"
...
class Settings:
    ...
    def __init__(self, path=default_path, parse_args=True):
        ...
        self.default_settings = {
            ...
            "OWNER": None,
            "PREFIXES": [],
            "default": {"ADMIN_ROLE": "Discord General Manager",
                        "MOD_ROLE": "Moderator",
                        "PREFIXES": []}
                        }
    ...
    def parse_cmd_arguments(self):
        parser = argparse.ArgumentParser(description="Chronoxia - Discord Bot")
        parser.add_argument("--owner", help="ID of the owner. Only who hosts "
                                            "Chronoxia should be owner, this has "
        ...
        parser.add_argument("--admin-role", help="Role seen as admin role by "
                                                 "Chronoxia")
        parser.add_argument("--mod-role", help="Role seen as mod role by Chronoxia")
        ...
        parser.add_argument("--no-cogs",
                            action="store_true",
                            help="Starts Chronoxia with no cogs loaded, only core")
        parser.add_argument("--self-bot",
                            action='store_true',
                            help="Specifies if Chronoxia should log in as selfbot")
        ...
        parser.add_argument("--dry-run",
                            action="store_true",
                            help="Makes Chronoxia quit with code 0 just before the "
    ...
    @property
    def token(self):
        return os.environ.get("CHRONO_TOKEN", self.bot_settings["TOKEN"])
    ...
    @property
    def email(self):
        return os.environ.get("CHRONO_EMAIL", self.bot_settings["EMAIL"])
    ...
    @property
    def password(self):
        return os.environ.get("CHRONO_PASSWORD", self.bot_settings["PASSWORD"])
